...
export default function Library() {
  ...
  return (
    <div className="max-w-5xl mx-auto px-3 md:px-0 pb-10">
      ...
      {/* Two summary cards side-by-side: Offline + Uploads */}
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-5">
        <Link
          to="/downloads"
          className="block rounded-2xl ring-1 ring-inset ring-border bg-gradient-to-br from-foreground/[0.06] to-foreground/[0.02] hover:from-foreground/[0.09] hover:to-foreground/[0.04] transition p-4 group"
        >
          <div className="flex items-center gap-3.5">
            <div className="w-10 h-10 rounded-xl bg-foreground/[0.06] grid place-items-center shrink-0">
              <CloudOff size={18} className="text-foreground/60" />
            </div>
            <div className="min-w-0 flex-1">
              ...
              <p className="text-xs text-foreground/55 mt-0.5">Listen without Wi‑Fi or data.</p>
            </div>
        ...
        >
          <div className="flex items-center gap-3.5">
            <div className="w-10 h-10 rounded-xl bg-foreground/[0.06] grid place-items-center shrink-0">
              <Upload size={18} className="text-foreground/60" />
            </div>
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-2">
                <span className="text-sm font-extrabold tracking-tight">Your Uploads</span>
                {uploads?.length > 0 && (
                  <span className="px-1.5 py-0.5 rounded-full bg-foreground/10 text-foreground/60 text-[10px] font-bold">
                    {uploads.length} tracks
                  </span>
                )}
              </div>
              <p className="text-xs text-foreground/55 mt-0.5">Music you've shared with PUBLIC.</p>
            </div>
            <ChevronRight size={18} className="text-foreground/40 group-hover:translate-x-0.5 transition shrink-0" />
          </div>
        </Link>
