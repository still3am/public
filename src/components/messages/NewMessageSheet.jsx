        ...
        <div className="flex-1 overflow-y-auto px-2 py-2 pb-[calc(0.5rem+env(safe-area-inset-bottom))]">
          {loading ? (
            ...
            <p className="text-center text-sm text-foreground/40 py-8">
              No users found. Try searching by name.
            </p>
          ) : !searched ? (
            ...
            results.map((u) => (
              ...
              >
                {u.avatar_url ? (
                  <img src={u.avatar_url} alt="" className="w-[52px] h-[52px] rounded-full object-cover shrink-0" />
                ) : (
                  <div className="w-[52px] h-[52px] rounded-full bg-foreground/10 grid place-items-center text-lg font-bold text-foreground/50 shrink-0">
