...
export default async function(req) {
  try {
    ...
    try {
      ...
      // Fallback: server may not support $exists — just classify tracks marked Other
    ...
    for (const track of tracks) {
      ...
      if (genre !== track.genre) {
        await base44.asServiceRole.entities.Track.update(track.id, { genre, last_updated_by_id: user.id });
      }
      processed++;
    }

    return Response.json({ processed, has_more: tracks.length === BATCH });
  } catch (error) {
